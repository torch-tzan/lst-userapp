...
const LstPaymentDetailAdmin = () => {
    ...
    return (
      <AdminLayout role="lst">
        <AdminPageHeader
          ...
          breadcrumbs={[
            { label: "LST HQ" },
            { label: "支払い履歴", to: "/admin/lst/payments" },
            { label: id ?? "—" },
          ]}
        />
        <div className={cardCls}>
          ...
          <Button className="mt-4" variant="outline" onClick={() => navigate("/admin/lst/payments")}>
            支払い一覧へ戻る
          </Button>
        </div>
      </AdminLayout>
  ...
  const affId = getPaymentAffiliateId(payment.id);
  const affName = affiliates.find((a) => a.id === affId)?.storeName ?? affId;
  ...
  return (
    <AdminLayout role="lst">
      ...
      <div className="mx-auto max-w-[1000px] space-y-6">
        <div className={cardCls}>
          <SectionHeader title="所属加盟店" />
          <div className="grid grid-cols-[120px_1fr_120px_1fr] gap-y-3 gap-x-4">
            <InfoRow label="加盟店">
              <button
                type="button"
                onClick={() => navigate(`/admin/lst/affiliates/${affId}`)}
            ...
            <InfoRow label="加盟店ID">
              <span className="font-mono text-xs">{affId}</span>
            </InfoRow>
          </div>
        </div>
        ...
        <div className={cardCls}>
          ...
          {payment.relatedTxId ? (
            <span className="text-sm text-slate-700">
              関連取引 ID: <span className="font-mono text-blue-600">{payment.relatedTxId}</span>
            </span>
          ) : (
...
export default LstPaymentDetailAdmin;
