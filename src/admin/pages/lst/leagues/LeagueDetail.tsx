...
const PlayerChip = ({ userId }: { userId: string }) => {
  ...
  return (
    <button
      type="button"
      onClick={() => navigate(`/admin/lst/leagues/players/${userId}`)}
      className="inline-flex items-center gap-1.5 rounded-full border border-slate-200 bg-white px-2.5 py-1 text-xs font-medium text-slate-700 transition-colors hover:bg-slate-50"
    >
    ...
    return (
      <AdminLayout role="lst">
        ...
        <div className={cardCls}>
          ...
          <Button className="mt-4" variant="outline" onClick={() => navigate("/admin/lst/leagues")}>
            リーグ一覧へ戻る
          </Button>
        </div>
      </AdminLayout>
